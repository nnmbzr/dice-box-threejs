...
const defaultConfig = {
	...
	strength: 1,
	iterationLimit: 1000,
	throwDirection: null, // направление броска в радианах
	throwPower: 1, // сила броска от 0 до 1
	onRollComplete: () => {},
	onRerollComplete: () => {},
	onAddDiceComplete: () => {},
...
class DiceBox {
	...
	loadAudio(src) {
		return new Promise((resolve, reject) => {
			...
			console.error("Unable to load audio", e);
		});
	}
	...
	startClickThrow(notation, options) {
		...
		if (options.direction !== undefined) {
			// Используем предоставленное направление
			vector = this.vectorFromDirection(options.direction);
			// Масштабируем вектор к размерам контейнера
			vector.x *= this.display.currentWidth;
			vector.y *= this.display.currentHeight;
		} else {
			// Используем случайное направление как раньше
			vector = {
		...
		const power =
			options.power !== undefined ? Math.max(0, Math.min(1, options.power)) : 1;
		let boost = (Math.random() + 3) * dist * this.strength * power;
	...
	async roll(notationString, options = {}) {
		this.notationVectors = this.startClickThrow(notationString, options);
	...
	async add(notationSting, options = {}) {
		let dieCount = this.diceList.length;
		if (!dieCount) return this.roll(notationSting, options);

		let addNotationVectors = this.startClickThrow(notationSting, options);
		let diceIdArray = [];
